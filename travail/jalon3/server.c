...
void handle_create_channel(ClientNode* client, const char* channel_name) {
    ...
    if (channel_exists(channel_name)) {
        ...
        send(client->pfd.fd, &response_msg, sizeof(response_msg), 0);
        return; // Arrêtez l'exécution ici si le salon existe déjà
    } else {
...
void handle_file_request(ClientNode* sender, const char* receiver_nickname, const char* file_name) {
    for (ClientNode* tmp = head; tmp; tmp = tmp->next) {
        if (strcmp(tmp->nickname, receiver_nickname) == 0) {
            struct message file_request_msg;
            memset(&file_request_msg, 0, sizeof(file_request_msg));
            file_request_msg.type = FILE_REQUEST;
            strncpy(file_request_msg.nick_sender, sender->nickname, NICK_LEN - 1);
            strncpy(file_request_msg.infos, file_name, INFOS_LEN - 1);

            send(tmp->pfd.fd, &file_request_msg, sizeof(file_request_msg), 0);
            break;
        }
    }
}

void handle_file_response(ClientNode* sender, struct message* msg) {
    for (ClientNode* tmp = head; tmp; tmp = tmp->next) {
        if (strcmp(tmp->nickname, msg->nick_sender) == 0) {
            send(tmp->pfd.fd, msg, sizeof(struct message), 0);
            break;
        }
    }
}
...
void handle_new_connection(int sfd) {
    ...
    // Création et envoi du message de confirmation avec le nouveau pseudo
    struct message response_msg;
...
void echo_server(ClientNode* client) {
    ...
    if (msgstruct.type == NICKNAME_NEW) {
        ...
        if (bytes_received <= 0) {
            ...
            for (ClientNode* tmp = head; tmp; tmp = tmp->next) {
                // Vérifiez si le client est dans le même salon et n'est pas l'expéditeur
                if (tmp != client && strcmp(tmp->channel_name, client->channel_name) == 0) {
                    ...
                    strncpy(multicast_msg.nick_sender, client->nickname, NICK_LEN - 1); // Nom de l'expéditeur
                    strncpy(multicast_msg.infos, client->channel_name, CHANNEL_LEN - 1); // Nom du salon
                    multicast_msg.pld_len = strlen(multicast_message); // Longueur du payload

                    // Envoyez le nom du salon dans le champ infos et le message dans le payload
        ...
        recv(client->pfd.fd, file_name, sizeof(file_name), 0); // Recevoir le nom du fichier
        handle_file_request(client, msgstruct.infos, file_name);
    } else if (msgstruct.type == FILE_ACCEPT || msgstruct.type == FILE_REJECT) {
        handle_file_response(client, &msgstruct);
    } else {
